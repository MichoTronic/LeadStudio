...
- Firebase manual Jira pilot: `leadStudioManualJiraV4` revision `leadstudiomanualjirav4-00005-zow`, separate operational/acceptance gates both disabled, editor hidden, dedicated writer identity
...
- 2026-08-17: Split manual Jira operational and acceptance gates, fixed the dormant preview editor status-element error, and briefly enabled the operational path for a real same-key save on test row 6. Provider validation passed, the row version changed, and idempotent replay caused no second write; `Debug Log` rows 1590-1591 contain metadata-only STARTED/COMPLETE records. Because no controllable browser was connected and the manual/scheduled services do not yet share a distributed lock, the preview editor was hidden again and both gates were disabled in final revision `leadstudiomanualjirav4-00005-zow`.
...
- Before enabling Firebase manual Jira, serialize it with the whole-Sheet Scheduler writer and complete signed desktop/mobile browser acceptance of the preview editor.
