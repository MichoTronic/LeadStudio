...
- Firebase Function revision `leadstudioactionv4-00013-lax` runs the settings-only, PII-minimized refresh dry-run with GAS-equivalent operational Gmail pagination and a private 35-column append payload. Live acceptance traversed both lead and onboarding query sets completely, matched all 18 accepted lead notices and all 27 onboarding notices, loaded 302 Sheet leads, planned no append/onboarding/Jira-status changes, and left both the Sheet and `Debug Log` unchanged. Mutations and scheduling remain disabled.
...
Keep GAS v60 as the operational baseline while V4 is migrated in controlled slices. The current Firebase preview is `https://timeless-lead-studio--v4-firebase-pilot-l3jpap21.web.app`; it reads the existing lead and Form-linked onboarding Sheets after central Auth, has verified keyless Gmail/Jira read parity, supports client-side filtered exports, and can build a bounded read-only refresh plan. Firebase write callables remain disabled after reversible acceptance: Firebase owns no operational writes, Jira synchronization, or scheduled triggers.
