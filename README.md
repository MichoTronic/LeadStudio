...
- Firebase Function revision `leadstudioactionv4-00012-yip` adds a settings-only, PII-minimized refresh dry-run. It composes the current Sheet snapshot with bounded Gmail lead/onboarding scans, Form-linked onboarding rows, and bulk Jira statuses without enabling writes or scheduling. Live acceptance loaded 302 leads, planned zero lead/onboarding changes and zero Jira status changes, identified 55 Jira timestamp candidates, and left both the Sheet and `Debug Log` unchanged.
