...
- The Firebase preview exports its currently filtered contacts as CSV or XLSX with the legacy visible-column contract. `leadStudioManualJiraV4` ports manual Jira linking behind separate operational/acceptance gates on the dedicated writer identity. Both reversible acceptance and a same-key operational row-6 save/replay passed. The preview editor and operational gate are temporarily enabled for signed browser QA; the acceptance gate remains disabled and GAS v60 remains the manual rollback UI.
...
The current Firebase preview is `https://timeless-lead-studio--v4-firebase-pilot-l3jpap21.web.app`; it reads the existing lead and Form-linked onboarding Sheets after central Auth, supports filtered exports, and shares the production refresh planner. Its manual Jira editor and operational gate are temporarily enabled for signed QA; all acceptance and callable refresh gates remain disabled. The dedicated 06:00 Scheduler remains the only automatic writer; GAS v60 has no trigger and must be treated as rollback-only for refreshes.
